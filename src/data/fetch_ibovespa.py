"""
Coleta série histórica diária do Ibovespa via Brapi com fallback Yahoo.

Saídas:
    - data/processed/ibovespa.parquet
    - data/external/{brapi|yahoo_finance}/ibovespa_*.parquet

Execute a partir da raiz do projeto:
    python -m src.data.fetch_ibovespa
"""
...
def fetch_ibovespa(range_: str = "10y", interval: str = "1d") -> Tuple[pd.DataFrame, str]:
    """Tenta coletar Ibovespa via Brapi, caindo para Yahoo se necessário."""
    config: Config = get_config()
    ticker = config.analysis.ticker_mercado

    # Brapi aceita encoding do '^' como %5E
    brapi_ticker = ticker.replace("^", "%5E")
    ...
    try:
        ...
        if not df_brapi.empty:
            logger.info("Ibovespa obtido via Brapi")
            return df_brapi, "brapi"
    except Exception as exc:  # noqa: BLE001
        logger.warning("Falha na Brapi para Ibovespa: %s", exc)
    ...
    if df_yahoo.empty:
        raise RuntimeError("Nenhuma fonte retornou dados do Ibovespa")

    logger.info("Ibovespa obtido via Yahoo Finance")
    return df_yahoo, "yahoo_finance"


def save_outputs(df: pd.DataFrame, source: str, processed_name: str = "ibovespa") -> Path:
    """Salva dataset processado e cópia bruta na área external."""
    config = get_config()
    
    # Output directory
    output_dir = config.paths.data_processed / "ibovespa"
...
def run() -> None:
    """Executa a coleta e persistência do Ibovespa."""
    logging.basicConfig(level=logging.INFO)
    config = get_config()
    start = pd.to_datetime(config.analysis.data_inicio)
    end = pd.to_datetime(config.analysis.data_fim)

    raw_df, source = fetch_ibovespa()
    standardized = _standardize_prices(raw_df, config.analysis.ticker_mercado)
    ...
    logger.info(
        "Cobertura Ibovespa: %s linhas, %s a %s, %s dias úteis faltantes",
